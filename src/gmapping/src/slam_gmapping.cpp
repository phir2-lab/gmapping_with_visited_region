...
void SlamGMapping::init()
{
  ...
  // VISITED REGION UPDATE
  private_nh_.getParam("output_file_address", output_file_address_);
  if(output_file_address_.compare("") != 0) gsp_->CreateOutputStream(output_file_address_.c_str());
  ...
  // VISITED REGION UPDATE
  if(!private_nh_.getParam("visited_radius", visited_radius_))
    visited_radius_ = 10;
...
void SlamGMapping::startLiveSlam()
{
  ...
  // VISITED REGION UPDATE
  visited_sst_ = node_.advertise<nav_msgs::OccupancyGrid>("map_visited", 1, true);
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
  ...
  matcher.setVisitedRadius(visited_radius_); // VISITED REGION UPDATE
  ...
  if(!got_map_) {
    ...
    // VISITED REGION UPDATE
    visited_map_.map.info.resolution = delta_;
    visited_map_.map.info.origin.position.x = 0.0;
    visited_map_.map.info.origin.position.y = 0.0;
    visited_map_.map.info.origin.position.z = 0.0;
    visited_map_.map.info.origin.orientation.x = 0.0;
    visited_map_.map.info.origin.orientation.y = 0.0;
    visited_map_.map.info.origin.orientation.z = 0.0;
    visited_map_.map.info.origin.orientation.w = 1.0;
  } 
  ...
  if(map_.map.info.width != (unsigned int) smap.getMapSizeX() || map_.map.info.height != (unsigned int) smap.getMapSizeY()) {
    ...
    // VISITED REGION UPDATE
    visited_map_.map.info.width = smap.getMapSizeX();
    visited_map_.map.info.height = smap.getMapSizeY();
    visited_map_.map.info.origin.position.x = xmin_;
    visited_map_.map.info.origin.position.y = ymin_;
    visited_map_.map.data.resize(map_.map.info.width * map_.map.info.height);
  ...
  for(int x=0; x < smap.getMapSizeX(); x++)
  {
    for(int y=0; y < smap.getMapSizeY(); y++)
    {
      ...
      // VISITED REGION UPDATE
      if(smap.cell(p).IsVisited())
        visited_map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = 0;
      else
        visited_map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = -1;
  ...
  // VISITED REGION UPDATE
  visited_map_.map.header.stamp = ros::Time::now();
  visited_map_.map.header.frame_id = tf_.resolve( map_frame_ );
  visited_sst_.publish(visited_map_.map);
