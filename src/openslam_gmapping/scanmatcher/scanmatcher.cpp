...
double ScanMatcher::registerScan(ScanMatcherMap& map, const OrientedPoint& p, const double* readings){
    ...
    // VISITED REGION UPDATE
    if(visited_mask_ready_)
    {
        int mask_size = visited_radius_*2+1;
        IntPoint robot = map.world2map(lp);
        IntPoint robot_2 = robot;
        for(int x=-visited_radius_; x<=visited_radius_; x++)
        {
            for(int y=-visited_radius_; y<=visited_radius_; y++)
            {
                if(visited_mask_[(visited_radius_+x)+((visited_radius_+y)*mask_size)] > 0)
                {
                    robot.x = robot_2.x+x;
                    robot.y = robot_2.y+y;
                    map.cell(robot).TurnToVisited();
                }
            }
        }
    }
...
void ScanMatcher::setVisitedRadius(int radius)
{
    visited_radius_ = radius;

    int m_maskSize = visited_radius_*2+1;

    //Calculating mask length
    int m_maskLength = m_maskSize*m_maskSize;

    //Alloc memory to the mask
    visited_mask_ = new int[m_maskLength];
    ...
    for(int y=0; y < m_maskSize; y++)
    {
        for(int x=0; x < m_maskSize; x++)
        {
            ...
            float distance = sqrt((x-visited_radius_)*(x-visited_radius_)+(y-visited_radius_)*(y-visited_radius_));
            int mask_value = ((distance <= visited_radius_+.5) ? 1 : 0);
            visited_mask_[x+m_maskSize*y] = mask_value;
        }
    }

    visited_mask_ready_ = true;
}
