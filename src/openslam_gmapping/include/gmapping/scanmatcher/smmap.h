...
struct PointAccumulator{
	...
	*/
	/*after begin*/
    PointAccumulator(): acc(0,0), visited(false), n(0), visits(0){}
    PointAccumulator(int i): acc(0,0), visited(false), n(0), visits(0){assert(i==-1);}
	/*after end*/
    ...
    inline void add(const PointAccumulator& p) {acc=acc+p.acc; n+=p.n; visits+=p.visits; visited=p.visited;}
    ...
    bool visited;
    inline void TurnToVisited();
    inline bool IsVisited();
};

// VISITED REGION UPDATE
void PointAccumulator::TurnToVisited()
{
    visited = true;
}

// VISITED REGION UPDATE
bool PointAccumulator::IsVisited()
{
    return visited;
}
